...
def main():
    ...
    field1 = mp.Field(mesh=msh, fieldID=mp.DataID.FID_Temperature, valueType=mp.ValueType.Scalar, unit=mp.U.K, time=1.*mp.U.s, value=values1)
    # field1.field2Image2D(title='Field', barFormatNum='%.0f')
    # field2 is cell based, i.e., field values are provided for cells
    field2 = mp.Field(
        mesh=msh, fieldID=mp.DataID.FID_Temperature, valueType=mp.ValueType.Scalar, unit=mp.U.K, time=1.*mp.U.s, value=values2, fieldType=mp.FieldType.FT_cellBased
    )
