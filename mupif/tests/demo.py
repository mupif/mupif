            ...
            if (self.field == None):
                ...
                for ix in range (self.nx+1):
                    for iy in range (self.ny+1):
                        ...
                        values.append((val,))
                self.field=Field.Field(self.mesh, FieldID.FID_Temperature, ValueType.ValueType.Scalar, temperatureUnit, time, values)
...
class AppMinMax(Application.Application):
    ...
    def getProperty(self, propID, time, objectID=0):
        if (propID == PropertyID.PID_Demo_Min):
            return Property.ConstantProperty(self._min, PropertyID.PID_Demo_Min, ValueType.ValueType.Scalar, propID, PQ.getDimensionlessUnit(), time=time)
        elif (propID == PropertyID.PID_Demo_Max):
            return Property.ConstantProperty(self._max, PropertyID.PID_Demo_Max, ValueType.ValueType.Scalar, propID, PQ.getDimensionlessUnit(), time=time)
        else:
...
class AppIntegrateField(Application.Application):
    ...
    def getProperty(self, propID, time, objectID=0):
        if (propID == PropertyID.PID_Demo_Integral):
            return Property.ConstantProperty(float(self.integral), PropertyID.PID_Demo_Integral, ValueType.ValueType.Scalar, propID, PQ.getDimensionlessUnit(), time=time)
        elif (propID == PropertyID.PID_Demo_Volume):
            return Property.ConstantProperty(float(self.volume), PropertyID.PID_Demo_Volume, ValueType.ValueType.Scalar, propID, PQ.getDimensionlessUnit(),time=time)
        else:
            raise APIError.APIError ('Unknown property ID')
...
class AppCurrTime(Application.Application):
    ...
    def getProperty(self, propID, time, objectID=0):
        if (propID == PropertyID.PID_Concentration):
            return Property.ConstantProperty(self.value, PropertyID.PID_Concentration, ValueType.ValueType.Scalar, 'kg/m**3', time=time)
        if (propID == PropertyID.PID_Velocity):
            return Property.ConstantProperty(self.value, PropertyID.PID_Velocity, ValueType.ValueType.Scalar, 'm/s', time=time)
        else:
            raise APIError.APIError ('Unknown property ID')
    def solveStep(self, tstep, stageID=0, runInBackground=False):
...
class AppPropAvg(Application.Application):
    ...
    def getProperty(self, propID, time, objectID=0):
        if (propID == PropertyID.PID_CumulativeConcentration):
            return Property.ConstantProperty(self.value/self.count, PropertyID.PID_CumulativeConcentration, ValueType.ValueType.Scalar, 'kg/m**3', time=time)
        else:
            raise APIError.APIError ('Unknown property ID')
