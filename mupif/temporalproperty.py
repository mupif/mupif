...
class TemporalProperty(Property):
    ...
    def _check_times(self, times):
        ...
        try:
            self.times.to('s')  # raise exception if not time dimension
        except Exception as e:
            raise ValueError('times does not have time units? ({e.message})')

    def _check_consistency(self):
        if np.any(self.times.shape[0] != self.quantity.shape[0]):
            raise ValueError(f'*times* and *quantity* must have the same first dimension (times: {self.times.shape[0]}, quantity: {self.quantity.shape[0]})')

    def evaluate(self, time: Quantity):
        if self.times.shape[-1] < 1:
            raise ValueError('No time points defined.')
    ...
    def to_db_dict(self):
        return {
            ...
            'Times': self.times.inUnitsOf('s').value.tolist()
        }
