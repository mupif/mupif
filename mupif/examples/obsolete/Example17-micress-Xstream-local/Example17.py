...
def runTestCase(xst,mic):
  ...
  try:
    ...
    propEpsXstream = Property.ConstantProperty(cConf.startEmissivity, 
                                               ...
                                               ValueType.ValueType.Scalar, PQ.getDimensionlessUnit(),
                                               time, 0 )   
    if (cConf.debug):
    ...
    while ( (time.inUnitsOf('s').getValue()+eps) < cConf.targetTime ):
      ...
      while ( pos < len(p) ): # loop over macro locations
        ...
        for interface in range(usedInterfaces):
          ...
          propLocation = Property.ConstantProperty( p[pos+interface], MICPropertyID.PID_RVELocation, ValueType.ValueType.Vector, 'm')
          propT = Property.ConstantProperty( temp, MICPropertyID.PID_Temperature, ValueType.ValueType.Scalar, 'K', time, 0 )
          # z-gradient constant at 0.0 at the moment
          propzG = Property.ConstantProperty( 0.0, MICPropertyID.PID_zTemperatureGradient, ValueType.ValueType.Scalar, 'K/m', time=None, objectID=0 )
      ...
      fieldEmissivity = Field.Field( bgMesh, XStFieldID.FID_Emissivity, \
                          ValueType.ValueType.Scalar, 'none', 0.0, emissivityValues )
      #if cConf.debug:
